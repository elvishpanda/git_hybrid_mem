...
//如果blk不是表头也不是表尾
	...
	{
//	printf("!blk->way_prev && !blk->way_next\n");
...
//blk是表尾(没有后继),重新设定set的表尾(blk的前驱),把blk独立出来
	...
	{
//	printf("!blk->way_next\n");
//		printf("set->way_head = 0x%x\n", set->way_head);
//		printf("set->way_tail = 0x%x\n", set->way_tail);
...
//新替换的块要被放到表头
	if (where == cache_waylist_head)
	{
//	printf("where == cache_waylist_head\n");
...
{
//	printf("here is hybrid_update_waylist\n");
	if (!blk->way_prev && !blk->way_next)
        {
//	printf("!blk->way_prev && !blk->way_next\n");
                assert(set->way_head == blk && set->way_tail == blk);
...
//找到要插入的位置的后继
		struct cache_block_t *last_forth = set->way_tail->way_prev->way_prev->way_prev->way_prev->way_prev;
...
{
//这个函数没有被调用
printf("here is cache_find_block\n");
	int set, tag, way;
	/*定位块*/
	/* Locate block */
...
void cache_access_block(struct cache_t *cache, int set, int way, int tag)
{
...
//更新缓存队列，把命中的缓存块放到LRU队列首部
	if (move_to_head && cache->sets[set].blocks[way].way_prev)
		cache_update_waylist(&cache->sets[set],
			&cache->sets[set].blocks[way],
			cache_waylist_head);
	else if(cache->policy == cache_policy_hybrid && cache->sets[set].blocks[way].way_prev)
	{
...
//DRAM SPACE
        if(backto_addr > 0x00000000 && backto_addr < 0x1FFFFFFF)
		return 0;
//PCM SPACE
        else if(backto_addr > 0x20000000 && backto_addr < 0xFFFFFFFF)
		return 1;
//WRONG SPACE
        else
		return -1;
